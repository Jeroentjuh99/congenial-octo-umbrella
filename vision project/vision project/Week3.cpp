...
Week3::Week3()
{
	image = imread("D:\\Users\\Remco\\Documents\\Github\\congenial-octo-umbrella\\vision project\\vision project\\test-images\\dice3.jpg", CV_LOAD_IMAGE_COLOR);
}
...
void Week3::BlobDetectie()
{
	...
	Mat binaryImage;
	threshold(gray_image, binaryImage, 75, 1, THRESH_BINARY_INV);
...
void Week3::BlobDetectieV2()
{
	...
	Mat binaryImage;
	threshold(gray_image, binaryImage, 190, 1, THRESH_BINARY);

	Mat kernel = Mat::ones(Size(2,2), CV_16S);
	Mat erodedImage;
	erode(binaryImage, erodedImage, kernel);

	Mat binary16S;
	erodedImage.convertTo(binary16S, CV_16S);
